...
def _check_nvenc_support() -> bool:
    try:
        result = subprocess.run([_get_ffmpeg(), '-encoders'], capture_output=True, text=True)
...
def _run_ffmpeg(args: list, desc: str = "", timeout: int = None):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            logging.error(f'ffmpeg error ({desc}): {result.stderr[-500:]}')
    ...
    try:
        ...
        for line in result.stderr.splitlines():
            ...
            if m_e and start is not None:
                silence.append((start, float(m_e.group(1)))); start = None
...
def download_and_probe_all(streams: dict, directory: str, max_workers: int = None):
    ...
    with create_shared_client() as client:
        with tqdm.tqdm(total=len(all_clips), desc="Processing files", unit="file") as pbar:
...
def compute_layout_timeline(streams: dict, total_duration: float, admin_user_id: Optional[int], hide_silent: bool = False, speech_timelines: Optional[dict] = None, start_time: float = 0, slide_timeline: Optional[list] = None) -> list:
    ...
    for i in range(len(filtered)-1):
        ...
        slide = None
        if not screenshare and slide_timeline:
            slide = next((path for s_s, s_e, path in slide_timeline if s_s <= t_m < s_e), None)
...
def _render_segment(seg_index: int, segment: dict, streams: dict, tmpdir: str, threads: int = 1, use_nvenc: bool = False) -> str:
    ...
    if not inputs:
        _run_ffmpeg(['-threads', str(threads), '-y', '-f', 'lavfi', '-i', f'color=black:s={OUTPUT_WIDTH}x{OUTPUT_HEIGHT}:r={OUTPUT_FPS}:d={duration:.3f}', '-f', 'lavfi', '-i', 'anullsrc=r=44100:cl=stereo', '-t', f'{duration:.3f}', '-c:v', video_encoder, '-preset', encoder_preset, '-c:a', 'aac', output_path], desc=f'black {seg_index}')
        return output_path
    ...
    for idx, type in v_inputs:
        if type == 'main':
            ...
            filter_parts.append(f'[{curr_v}][{idx}:v]overlay={MAIN_WIDTH}:{ov_idx*SIDEBAR_HEIGHT}:eof_action=repeat[v{ov_idx}]')
        curr_v = f'v{ov_idx}'; ov_idx += 1

    if a_inputs:
        for i, idx in enumerate(a_inputs):
            filter_parts.append(f'[{idx}:a]asetpts=PTS-STARTPTS,aresample=44100,apad=whole_dur={duration:.3f}[a{i}]')
    ...
    cmd += ['-filter_complex', ';'.join(filter_parts), '-map', f'[{curr_v}]', '-map', a_map, '-c:v', video_encoder, '-preset', encoder_preset]
    if is_min and not use_nvenc: cmd += ['-crf', '45']
    if not use_nvenc: cmd += ['-tune', 'stillimage' if is_min else 'zerolatency']
    cmd += ['-pix_fmt', 'yuv420p', '-r', str(OUTPUT_FPS), '-c:a', 'aac', '-t', f'{duration:.3f}', output_path]
...
def render_all_segments(timeline: list, streams: dict, tmpdir: str) -> list:
    ...
    max_workers = min(cpu_count, 12) if use_nvenc else (32 if cpu_count >= 60 else min(cpu_count, 8))
    ...
    try:
        with tqdm.tqdm(total=len(timeline), desc="Rendering", unit="seg") as pbar:
            executor = ThreadPoolExecutor(max_workers=max_workers)
            try:
                futures = {executor.submit(_render_segment, i, s, streams, tmpdir, threads_per_worker, use_nvenc): i for i, s in enumerate(timeline)}
                for f in as_completed(futures):
                    idx = futures[f]
                    try: segment_files[idx] = f.result()
                    except Exception as e:
                        if 'interrupted' not in str(e): logging.error(f'Seg {idx} failed: {e}')
                    pbar.update(1)
            except KeyboardInterrupt:
                executor.shutdown(wait=False, cancel_futures=True); raise
            finally: executor.shutdown(wait=True)
    finally: root_logger.handlers = old_handlers
...
def process_composite_video(directory: str, json_data: dict, output_path: str, max_duration=None, hide_silent: bool = False, start_time: float = 0):
    ...
    if slides:
        from .webinar import create_directory_if_not_exists # avoid circular
        slide_map = {}
        with create_shared_client() as client:
            for i, url in enumerate(list(dict.fromkeys(slides))):
                ...
                slide_map[url] = path
        s_timeline = [(s, e, slide_map[u]) for s, e, u in s_timeline if u in slide_map]
