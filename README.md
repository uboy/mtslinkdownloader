...
Также можно выбрать уровень логирования (`DEBUG`, `INFO`, `WARNING`, `ERROR`) прямо в интерфейсе.
...
- Камера лектора старается оставаться первой в правой панели; признак `ADMIN` используется как эвристика (tie-break), а не как единственный источник истины.
- Камеры участников отрисовываются в правой панели сеткой 2x2; первый говорящий участник появляется в правом верхнем слоте рядом с лектором.
- Нижняя часть правой панели выделена под чат/Q&A: показываются последние сообщения, список обновляется по мере появления новых сообщений.
- Сообщения в чате показываются с таймстампом и отправителем в формате `[CHAT HH:MM:SS] username` / `[Q&A HH:MM:SS] username`, а текст переносится по ширине блока.
- Блок чата рисуется внутри правой панели, без перекрытия слайда; по ширине он чуть меньше суммы двух камер (с учетом промежутка между ними).
...
- Таймлайн презентации строится по URL/индексу слайда из событий `presentation.*`, чтобы избежать рассинхрона слайдов.
- Длительность каждого медиа-потока (камеры, демонстрация экрана) теперь строго ограничивается событиями удаления (`conference.stream.delete`, `screensharing.stream.delete`), что предотвращает «зависание» старых кадров поверх актуальных слайдов.
- Для основного холста source выбирается по сегментам: `screenshare` используется только если активный clip имеет видео; иначе в основном холсте показывается слайд из `presentation.*`.
- При отсутствии `conference.id` в медиасессии используется fallback stream key (`fallback:<...>`), чтобы не схлопывать разные источники в один поток.
...
Для настройки подробности логов используйте `--log-level`:

```bash
mtslinkdownloader <URL> --log-level DEBUG
```
